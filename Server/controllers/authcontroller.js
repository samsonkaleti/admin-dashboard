...
exports.signup = async (req, res) => {
    ...
    try {
      ...
      return res.status(200).json({ message: 'User registered successfully, OTP sent to email', userId: newUser._id });
    } catch (error) {
  ...
  exports.verifyOTP = async (req, res) => {
    ...
    try {
      ...
      const otpEntry = user.otp.find(entry => entry.code === otp && entry.expiration > Date.now());
...
exports.login = async (req, res) => {
  ...
  try {
    ...
    if (!user.isVerified) {
      return res.status(400).json({ message: 'User not verified. Please verify the OTP sent to your email' });
    }
